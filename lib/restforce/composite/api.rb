require 'uri'
require 'restforce/composite/verbs'

module Restforce
  module Composite
    module API
      extend Restforce::Composite::Verbs
      ...
      # Returns an Array of String names for each SObject.
      def limits
        version_guard(29.0) { api_get("limits") }
      end
      ...
      def describe(sobject = nil)
        if sobject
          api_get("sobjects/#{sobject}/describe")
        else
      ...
      def describe_layouts(sobject, layout_id = nil)
        version_guard(28.0) do
          if layout_id
            api_get("sobjects/#{sobject}/describe/layouts/#{layout_id}")
          else
            api_get("sobjects/#{sobject}/describe/layouts")
          end
        end
      end
      ...
      #      ain.htm
      def explain(soql)
        version_guard(30.0) { api_get("query", explain: soql) }
      end
